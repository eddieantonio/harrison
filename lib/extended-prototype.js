...
exports.mainApp = function mainApp(options) {
  ...
  this._subapps.addMiddleware(this);
};
...
/*
 * TODO: consider exporting the middleware seperately...
 *
 * Maybe get rid of this... monkey-patching non-sense.
 */
SubappManager.prototype.addMiddleware = function (app) {
  var mountPath, appName, subApp, self = this;
  var staticPath;

  for (appName in self.apps) {
    subApp = self.apps[appName];
    /* For now, these are one and the same thing. */
    mountPath = '/static' + appName;

    staticPath = subApp.get('harrison static');
